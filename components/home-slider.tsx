...
const slides = [
  {
    image: "/HomeSlider/slider-1.jpg?height=600&width=1600",
  ...
  {
    image: "/HomeSlider/slider-2.jpg?height=600&width=1600",
  ...
  return (
    <div className="relative h-[600px] md:h-[700px] overflow-hidden">
      {slides.map((slide, index) => (
        ...
        >
          <Image src={slide.image || "/HomeSlider/slider-1.jpg"} alt={slide.title} fill className="object-cover brightness-[0.7]" priority={index === 0} />
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
            ...
            <div className={`transition-all duration-1000 delay-500 ${index === current ? "animate-zoom-in opacity-100" : "opacity-0 scale-90"}`}>
              <Button
                size="lg"
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex space-x-3 z-10">
        {slides.map((_, index) => (
          <button
            key={index}
            className={`h-3 w-3 rounded-full transition-all duration-300 ${index === current ? "bg-primary w-10" : "bg-white/50 hover:bg-white/80"}`}
            onClick={() => {
              if (isAnimating) return;
              setIsAnimating(true);
              setCurrent(index);
